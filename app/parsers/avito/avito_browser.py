...
async def _search(filters: Dict[str, Any], limit: int) -> List[dict]:
    ...
    proxy = ProxySettings.playwright_proxy()
    if proxy:
        launch["proxy"] = proxy
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            logger.info(f"AVITO PLAYWRIGHT STATUS {status}")
            ...
            raw = await page.evaluate(
                """() => {
                  ...
                  pool.forEach(node => {
                    const a = node.querySelector ? (node.querySelector('a[itemprop="url"], a[data-marker="item-title"], a[href*="_"]') || node) : node;
            ...
            for item in raw or []:
                ...
                if len(ads) >= limit:
                    break
            logger.info(f"AVITO PLAYWRIGHT FOUND: {len(ads)}")
