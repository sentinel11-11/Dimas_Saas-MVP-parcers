...
class DromParser(BaseParser):
    ...
    def __init__(self):
        self.client = HTTPClient(min_delay=0.3, max_delay=0.8, retry_count=2, use_proxy=False)
    ...
    def search(self, filters):
        ...
        for page in range(1, pages + 1):
            ...
            if not url:
                logger.warning("DROM skip: no brand/model")
                return []
    ...
    def parse_card(self, card):
        title = ""
        title_tag = card.find(attrs={"data-ftid": "bull_title"})
        if title_tag:
            title = title_tag.text.strip()
        price = 0
        price_tag = card.find(attrs={"data-ftid": "bull_price"})
        if price_tag:
        ...
        mileage = None
        mileage_match = re.search(r"(\d[\d\s]+)\s?км", card.text)
        if mileage_match:
            digits = re.sub(r"\D", "", mileage_match.group(1))
            if digits:
                mileage = int(digits)
    ...
    def _card_image(card) -> str:
        ...
        for img in card.find_all(["img", "source"]):
            for attr in ("src", "data-src", "data-lazy", "data-original", "data-src-original", "srcset", "data-srcset"):
                raw = img.get(attr) or ""
                if not raw:
                    continue
